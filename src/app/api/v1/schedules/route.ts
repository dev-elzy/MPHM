...
export async function GET(req: Request) {
  try {
    ...
    if (!academicYearId || !semesterId) {
      return NextResponse.json({ success: false, message: 'academicYearId and semesterId required' }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: true, message: 'Success', data: { items } });
  } catch (error: unknown) {
